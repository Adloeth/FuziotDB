...
namespace FuziotDB
{
    ...
    public struct DBVariant
    {
        ...
        public static DBVariant FromBytes(byte[] bytes, ushort size, DBType type)
        {
            ...
            if(type == DBType.UInt && size == 0 ) return new DBVariant(bytes[0]);
            if(type == DBType.UInt && size == 1 ) return new DBVariant(BitConverter.ToUInt16(bytes));
            if(type == DBType.UInt && size == 3 ) return new DBVariant(BitConverter.ToUInt32(bytes));
            if(type == DBType.UInt && size == 7 ) return new DBVariant(BitConverter.ToUInt64(bytes));
            if(type == DBType.UInt && size == 15) return new DBVariant(new Guid(bytes));
            if(type == DBType. Int && size == 0 ) return new DBVariant((sbyte)bytes[0]);
            if(type == DBType. Int && size == 1 ) return new DBVariant(BitConverter.ToInt16(bytes));
        ...
        public static unsafe DBVariant FromObjectBytes(byte* bytes, long offset, ushort size, DBType type)
        {
            ...
            if(type == DBType.UInt && size == 0 ) return new DBVariant(bytes[0]);
            if(type == DBType.UInt && size == 1 ) return new DBVariant(((ushort*)bytes)[0]);
            if(type == DBType.UInt && size == 3 ) return new DBVariant(((  uint*)bytes)[0]);
            if(type == DBType.UInt && size == 7 ) return new DBVariant((( ulong*)bytes)[0]);
            if(type == DBType.UInt && size == 15) return new DBVariant(((  Guid*)bytes)[0]);
            if(type == DBType. Int && size == 0 ) return new DBVariant((sbyte)bytes[0]);
            if(type == DBType. Int && size == 1 ) return new DBVariant((( short*)bytes)[0]);
